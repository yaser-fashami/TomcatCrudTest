...
namespace Mc2.CrudTest.Framework.EndPoints.WebMVC.Controllers;
public class BaseController : Controller
{
    ...
    {
        var result = await CommandDispatcher.Send<TCommand>(command);
        if (result.Status == ApplicationServiceStatus.Ok)
        {
    ...
    protected async Task<IActionResult> Edit<TCommand>(TCommand command)
        where TCommand : class, ICommand
    {
        var result = await CommandDispatcher.Send<TCommand>(command);
        if (result.Status == ApplicationServiceStatus.Ok)
